...
Post Request With Redirection
    ...
    ${resp}=  Post Request  ${SESSION}  /redirect-to?url=anything  data=something
    Status Should be  OK  ${resp}
    ${redirected_url}=  Catenate  ${HTTP_LOCAL_SERVER}/anything
    Should Be Equal As Strings  ${resp.json()['url']}  ${redirected_url}
    ${resp}=  Post Request  ${SESSION}  /redirect-to?url=anything  data=something  allow_redirects=${true}
    Status Should be  OK  ${resp}
    ${redirected_url}=  Catenate  ${HTTP_LOCAL_SERVER}/anything
    Should Be Equal As Strings  ${resp.json()['url']}  ${redirected_url}
...
Post Request Without Redirection
    [Tags]  post
    ${resp}=  Post Request  ${SESSION}  /redirect-to?url=anything  data=something  allow_redirects=${false}
